...
export function StudentSessionEnded({
  ...
  // If the leaderboard has updated data for this participant, use it to ensure the final score matches.
  const leaderboardEntry = leaderboard?.find(
    (entry) => entry.name === participantName && (entry.phoneNumber === phoneNumber || (!entry.phoneNumber && !phoneNumber))
  );
  const finalScore = leaderboardEntry?.score ?? participantSummary?.score ?? 0;
  const finalRank = leaderboardEntry?.rank ?? participantSummary?.rank;
    ...
    <div className="relative flex min-h-0 flex-1 flex-col overflow-y-auto overflow-x-hidden bg-[#0f172a] p-4 text-white sm:p-6">
      ...
      <div className="relative z-10 flex flex-1 flex-col items-center justify-start py-4 text-center">
        ...
        >
          ...
          <div className="mt-5 sm:mt-6 grid grid-cols-1 gap-2 sm:gap-3 md:grid-cols-3">
            ...
            <div className="rounded-[1.6rem] border border-emerald-400/20 bg-emerald-500/10 p-3 sm:p-4 text-left backdrop-blur-xl">
              ...
              <p className="mt-2 text-xl sm:text-2xl font-black text-white">{finalScore}</p>
            </div>
          </div>
