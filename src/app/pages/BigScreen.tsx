...
import { Users, QrCode as QrIcon, Timer, Trophy } from "lucide-react";
...
const MOCK_LEADERBOARD = [
  { name: "Sarah J.", score: 2450, rank: 1, avatar: "SJ" },
  { name: "Mike T.", score: 2380, rank: 2, avatar: "MT" },
  { name: "Alex R.", score: 2210, rank: 3, avatar: "AR" },
  { name: "Elena W.", score: 2150, rank: 4, avatar: "EW" },
  { name: "David K.", score: 1980, rank: 5, avatar: "DK" },
];
...
export function BigScreen() {
  ...
  // Synchronize with session context
  useEffect(() => {
    if (currentSession) {
      if (currentSession.status === "active") {
        setView("question");
        // Only reset timer if it's a new question or just started
        setTimeLeft(currentSession.questions[currentSession.currentQuestionIndex].timer);
      ...
      <header className="relative z-10 flex items-center justify-between mb-16">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-indigo-400 font-black uppercase tracking-[0.2em] text-sm mt-1">Join at quiz.pro with code</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-6 bg-white/5 backdrop-blur-2xl px-10 py-6 rounded-[2.5rem] border border-white/10 shadow-2xl">
            ...
            <div>
              <p className="text-5xl font-black text-white leading-none">{currentSession?.participants || 42}</p>
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <div className="relative group">
                ...
                <div className="relative bg-white p-12 sm:p-16 rounded-[3rem] shadow-[0_0_100px_rgba(79,70,229,0.3)]">
                  <QRCodeSVG value={`https://quiz.pro/join/${code}`} size={350} />
                </div>
              </div>
            ...
            >
              <div className="bg-white/5 backdrop-blur-3xl border border-white/10 p-16 rounded-[4rem] shadow-2xl relative overflow-hidden group">
                ...
                <h2 className="text-6xl font-black text-center pt-10 leading-tight tracking-tight">
                  {currentSession.questions[currentSession.currentQuestionIndex].text}
                </h2>
              </div>

              <div className="grid grid-cols-2 gap-10">
                {currentSession.questions[currentSession.currentQuestionIndex].options.map((opt, i) => (
                  <motion.div
                    key={i}
          ...
          {view === "results" && currentSession && (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-10">
                {currentSession.questions[currentSession.currentQuestionIndex].options.map((opt, i) => {
                  const isCorrect = i === currentSession.questions[currentSession.currentQuestionIndex].correctAnswer;
                  return (
                    <motion.div
                      key={i}
                      initial={{ opacity: 0 }}
              ...
              <div className="space-y-6">
                {MOCK_LEADERBOARD.map((item, i) => (
                  <motion.div
                    ...
                    className={`relative p-8 rounded-[2.5rem] border-2 flex items-center justify-between transition-all ${
                      i === 0 ? "bg-gradient-to-r from-amber-500/20 to-transparent border-amber-500/50 scale-105 shadow-2xl" :
                      i === 1 ? "bg-gradient-to-r from-slate-400/20 to-transparent border-slate-400/30 scale-[1.02]" :
                      i === 2 ? "bg-gradient-to-r from-amber-700/20 to-transparent border-amber-700/30 scale-[1.01]" :
                      "bg-white/5 border-white/5"
                    }`}
                  >
                    <div className="flex items-center gap-10">
                      <span className={`w-16 h-16 flex items-center justify-center font-black text-3xl rounded-[1.5rem] shadow-lg ${
                      ...
                      <div className="w-16 h-16 bg-indigo-500 rounded-full flex items-center justify-center font-black text-2xl border-4 border-[#0f172a] shadow-xl">
                        {item.avatar}
                      </div>
                      <span className="text-4xl font-black tracking-tight">{item.name}</span>
                    </div>
                    <div className="flex items-baseline gap-3">
                      <span className="text-5xl font-black text-indigo-400 tracking-tighter">{item.score}</span>
                      <span className="text-sm font-black uppercase text-gray-500 tracking-widest">PTS</span>
                    </div>
                  </motion.div>
                ))}
              </div>
      ...
      <footer className="relative z-10 flex items-center justify-between pt-16 border-t border-white/5 mt-auto">
        <div className="flex gap-6">
          <div className="px-8 py-4 bg-white/5 rounded-2xl border border-white/10 text-xs font-black uppercase tracking-[0.2em] text-gray-400">
            {currentSession?.questions.length || 0} ROUNDS TOTAL
          </div>
